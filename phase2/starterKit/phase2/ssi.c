#include "dep.h"
/* given an int int and an integer representing the bit you want to check
 (0-indexed) and return 1 if the bit is 1, 0 otherwise */
#define BIT_CHECKER(n, bit) (((n) >> (bit)) & 1)
struct pcb_t *Create_Process(pcb_t *sender, struct ssi_create_process_t *arg);
struct support_t *Get_Support_Data(pcb_t *sender);

void SSI_function_entry_point() {
  while (TRUE) {
    ssi_payload_PTR process_request_payload;
    pcb_t* process_request_ptr = (pcb_t*) SYSCALL(RECEIVEMESSAGE, ANYMESSAGE, (int)(&process_request_payload), 0);
...
void SSI_Request(pcb_t* sender, int service, void *arg) {
  ...
  int syscall_response_arg = 0;
  int has_response = TRUE;
  ...
  case GETSUPPORTPTR:
    ...
    break;
  case GETPROCESSID:
    syscall_response_arg = (int)Get_Process_ID(sender, (int)arg);
    break;
  default:
  ...
  if (has_response)
    SYSCALL(SENDMESSAGE, (unsigned)sender, syscall_response_arg, 0);
}
...
pcb_PTR Create_Process(pcb_t *sender, struct ssi_create_process_t *arg) {
  ...
  new_prole->p_time = 0;
  // enalbe interrupts
  new_prole->p_s.status = ALLOFF | IEPON | IMON | TEBITON;
  processCount++;
  insertProcQ(&ready_queue, new_prole);
  insertChild(sender, new_prole);
  return new_prole;
  
}

int Terminate_Process(pcb_t *sender, pcb_t *target) {
  /*
...
int DoIO(pcb_t *sender, ssi_do_io_PTR arg) {
  ...
  // int dev_no = 7;                       // *specs for terminal *
  int dev_line = IL_TERMINAL - 14;  // *                   *
  int dev_no = (int)arg->commandAddr - 0x10000054 - ((dev_line-3) * 0x80);
  dev_no = dev_no / 0x10;

  int dev_index = (dev_line - 3) * 8 + dev_no;

  pcb_t* blocked_for_message = outProcQ(&msg_queue, sender);
  ...
  if (blocked_for_message == NULL) {
    ...
    if ((blocked_for_message = outProcQ(&ready_queue, sender)) == NULL)
      return -1;
    else
      softBlockCount++;
  }
...
void Wait_For_Clock(pcb_t *sender) {
  ...
  insertProcQ(&PseudoClockWP, outProcQ(&msg_queue, sender));
...
void killProgeny(pcb_t *sender) {
  ...
  pcb_t* removed = outProcQ(&ready_queue, sender);

  if (outProcQ(&msg_queue, sender) != NULL || outProcQ(&PseudoClockWP, sender) != NULL) {
    softBlockCount--;
