...
// GET_VPN(x) estrae il numero di pagina virtuale da x e lo limita a un massimo di 31
...
void uTLB_RefillHandler() {
  ...
  // setto i registri ENTRYHI e ENTRYLO (della CPU) con i valori della pagina p
  ...
  // legge dai registri ENTRYHI e ENTRYLO appena caricati e scrive nella TLB
  TLBWR();
...
/**
 * Funzione per invalidare una pagina sporca
 */
 ...
 */
// serve per scrivere su backing store (device flash), tramite richiesta IO all'SSI
...
void pager() {
  ...
  //mando una richiesta al ssi per ottenere il puntatore alla support struct e attendo risposta
  ...
  if ((cause & GETEXECCODE) >> CAUSESHIFT == 1) {
    ...
    int i = getPage(); // indice della pagina vittima
    ...
    // frame appartenente ad un altro processo e in caso serve "pulirla" poichè
    ...
    if (swap_pool_entry->sw_asid != NOPROC) {
      ...
      // scrivi nel backing store/flash 
    ...
    */
    ...
    // FINE MUTUA ESCLUSIONE, ritorno il controllo al processo corrente per far riprovare
    // l'istruzione che aveva causato l'evento di Page Fault
